...
INT	i;
	...
	// $4700ÇÕë“ã@ñ≥å¿ÉãÅ[Év
	WRAM[0xA700] = 0x4c;	//jmp
	WRAM[0xA701] = 0x00;
	WRAM[0xA702] = 0x47;

	// $4710ÇÕInitÉãÅ[É`ÉìÇåƒÇÒÇæå„ñ≥å¿ÉãÅ[ÉvÇ÷
	...
	// $4720ÇÕPlayÉãÅ[É`ÉìÇåƒÇÒÇæå„ñ≥å¿ÉãÅ[ÉvÇ÷
#if	NSF_PROFILE
	...
	// NTSC/PAL tune
	nes->SetVideoMode( (nsfheader.NTSC_PALbits&0x01) );

	// é¸îgêî->KeyÉeÅ[ÉuÉãÇÃçÏê¨
	for( i = 0; i < 12*8+1; i++ ) {
...
BYTE	MapperNSF::ReadLow( WORD addr )
{
	...
	if (addr >= 0x4700 && addr < 0x4730) { //NSF player opcode “∂∑„
...
//	DrawString( 10, 61, "NO    :", 0x30 );
	...
	// ÉLÅ[É{Å[ÉhÇ™âüÇ≥ÇÍÇƒÇ¢ÇÈÇÃÇï`âÊ
	// Internal APU
